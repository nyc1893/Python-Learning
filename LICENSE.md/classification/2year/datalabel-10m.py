...
#Constants
    ...
    for i in range(0,nn):
        ...
        if os.path.exists(filename) ==1:
            ...
            elif k == 1 and df['vp_m'].shape[0] == num_time_sample:
                st.append(df['vp_m']) 
                
            elif k == 2 and df['ip_m'].shape[0] == num_time_sample:
                st.append(df['ip_m']) 
               
            # elif k == 2 and df['df'].shape[0] == 36000:
                # st.append(df['df']) 
                
            elif k == 3 and df['vp_a'].shape[0] == num_time_sample:
                st.append(df['vp_a'])    
 
            elif k == 4 and df['ip_a'].shape[0]== num_time_sample:
                st.append(df['ip_a'])   

                
            elif k == 5 and df['ip_a_diff_grad'].shape[0]== num_time_sample:
                st.append(df['ip_a_diff_grad'])   
 
            elif k == 6 and df['f'].shape[0]== num_time_sample:
                st.append(df['f'])   
