...
def base():
    ...
    # hyperparameters = dict(leaf_size=1, n_neighbors=17, p=1)
    baseline =  LogisticRegression(C=1.0, max_iter= 500, solver ='newton-cg',n_jobs=40)
...
def rev_res():
    ...
    dt2 = df2[df2["real"]!=df2["LR"]].index
    dt3 = jiaoji(dt1,dt2)
    
    df2.loc[dt3,"real"] = df2.loc[dt3,"LR"] 
