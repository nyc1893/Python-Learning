...
def cal_sub(ii):
    ...
    for j in range(x.shape[0]):
        ...
        ind3 = np.argmax(x2[j,0])
        ind4 = np.argmax(x3[j,0])
        
        sd.append(ind3+reg+20)
        sd.append(ind4+reg+20)
        x2[j,0,ind3-reg:ind3+reg] = -100
        x3[j,0,ind4-reg:ind4+reg] = -100
    ...
    list2 = [
            "v_up","v_dn","v_dup","v_ddn","v_aup","v_adn",
            "i_up","i_dn","i_dup","i_ddn","i_aup","i_adn",
            "r_up","r_dn","r_dup","r_ddn","r_aup","r_adn"
            ]
    list3 =[]
    ...
    name_list = ["line_ind","freq_ind","linesub_ind","freqsub_ind"]
    # print(list3)
    name_list = name_list +list3
    ...
    df.to_csv("data/Sub"+str(ii)+".csv", index = None)
    print("S"+str(ii)+" save done")
