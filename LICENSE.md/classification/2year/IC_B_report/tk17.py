...
def add_zero(a):
    if(len(a)<2):
        ...
        return a
def cc():
    df= pd.read_csv(path + "2017_test.csv")
    dt = df.values
    st = []
    for i in range(df.shape[0]):    
        ...
        st.append(x1[1]+"/"+x2[1]+"/2017 "+ x3[1]+":"+add_zero(x5[0]))
    ...
    df.to_csv("2017_2.csv",index = None)

def change_label():
    path = "orignal/"
    df= pd.read_csv(path+"2017B_test.csv")
    ...
    df.RF[ind1] = 2
    df.RF[ind2] = 3
    df.to_csv("2017_3.csv",index = None)
    
def deal_time():
    res =[]
    df= pd.read_csv("2017_3.csv")
    ...
    df["event"] = st2
    df.to_csv("2017_4.csv",index = None)
def main():
    cc()
    change_label()
    deal_time()
