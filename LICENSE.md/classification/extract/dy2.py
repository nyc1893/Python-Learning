# -*- coding: utf-8 -*-
"""
Created on Wed Jan 22 12:39:03 2020
...
# Protected main functio
    ...
    '''
    path1 = '/pnnl1/ic_b/theYear=2017/theMonth='+str(int(sys.argv[1]))+'/theDay='
...
#    df_iter = df_iter[5:7]
    ...
    for Days in range(int(sys.argv[2]),int(sys.argv[3])):
        path1 = r'/pnnl1/ic_b/theYear=2017/theMonth='+str(int(sys.argv[1]))+'/theDay='+str(Days)

        all_files = glob.glob(path1 + "/*.parquet")
        df_it=[]
        Event_data_list=[]
        for days in range(len(dd)):
            if (dd[days].day==Days) & (dd[days].month==int(sys.argv[1])) & (dd[days].year==2017):
                df_it.append(days)        
        if not len(df_it)==0:
