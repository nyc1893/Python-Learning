...
def save_raw(ii):  
    ...
    pickle_out = open(path3 + "X_S"+str(ii)+".pickle","wb")
    pickle.dump(a, pickle_out, protocol=2)
    pickle_out.close()    
...
def pre_olap(ii):  
    ...
    for i in range(a.shape[0]):
        temp = get_feature(a[i,:,:,:],60)

        st.append(temp)
    X_train =np.array(st)
    print(X_train.shape)
    
    path3 = 'ml2/dif_scal/60/'
    # save features for all the events
    
    pickle_out = open(path3 + "90s_nor_X_S"+str(ii)+".pickle","wb")
    pickle.dump(X_train, pickle_out, protocol=2)
    pickle_out.close()    
    
    yy = pd.DataFrame(y_train)
    yy.columns = ["label"]
    yy.to_csv(path3 +"90s_nor_y_S"+str(ii)+".csv",index =None)    
    print("save done S"+str(ii))
