...
def main():
    ...
    dim_learning_rate = Real(low=1e-5, high=1e-1, prior='log-uniform',
                             name='learning_rate')
    dim_num_dense_layers = Integer(low=3, high=6, name='num_dense_layers')
    dim_num_dense_nodes = Integer(low=2, high=3, name='num_dense_nodes')
    ...
    def fitness(learning_rate, num_dense_layers,
        ...
        callback_log = TensorBoard(
            ...
            write_images=False)
        path2= 'bodnn-'
        file_name= path2+ "best_model_weights.hdf5"
        ...
        if accuracy > best_accuracy:
            ...
            model.load_weights(path2+"best_model_weights.hdf5")# save best model's weights
            model.save(path2+"best_model_so_far.h5") # save the entire model configurartion and weights
            print('**** THE BEST ACCURACY SO FAR IS ACHIEVED AND THE MODEL IS SAVED **** \n')
            
            # save the hyperparameters
            pickle_out = open(path2+"hp.pickle","wb")
