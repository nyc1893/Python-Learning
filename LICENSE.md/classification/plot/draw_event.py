...
def test(num):
            ...
            if os.path.exists(filename) ==1:
                ...
                for i in range(0,df.shape[0]):
                    ...
                    if os.path.exists(path2+fname2+'.parquet') ==1:
                        ...
                        # st.append(df2['ip_m'][start_crop:stop_crop])  
                        if(df2['vp_a'].shape[0] == 36000):
                            # print('Yeah')
                            st.append(df2['vp_a'])  #get the 1st 1 mins data for X
                            # print(df2.head())
                            event_name = x[0]
    ...
    pickle_out = open('X'+str(num)+'_ip_msee.pickle',"wb")
    ...
    pickle_out = open('y'+str(num)+'_ip_msee.pickle',"wb")
    ...
    """    
def load_data(num):
    p1 = open('X'+str(num)+'_ip_msee.pickle',"rb")
    X_train= pickle.load(p1)
    print(X_train.shape)

    p2 = open('y'+str(num)+'_ip_msee.pickle',"rb")
    y= pickle.load(p2)
    y2 = pd.DataFrame(y)
...
def load_data2(num):
    ...
    y2 = pd.DataFrame(y)
    y2.columns = ['A','B']
    # df = y2[y2['A'].str.contains('Line')]
    df = y2[y2['A'].str.contains('XFMR')]
    # df = y2[y2['A'].str.contains('Freq')]
    # df = y2[y2['A'].str.contains('Os')]
    print(df.shape)
...
def draw2(p3):
    # print(p3[0].shape[0])
    # for i in range(0,p3[0].shape[0]-1):
        # print(p3[i].shape)
